...
import { Conversation, Client } from '@xmtp/node-sdk';
...
class HandlerContext {
    ...
    static async loadCommandConfig(configPath = "commands.js") {
        const resolvedPath = path__default.resolve(process.cwd(), "dist/" + configPath);
        try {
...
async function xmtpClient(config) {
    ...
    const v2client = await Client$1.create(wallet2, {
        ...finalConfig,
        apiUrl: undefined,
    ...
    if (config?.experimental) {
        console.warn("\x1b[33m⚠️  WARNING: Experimental mode enabled ⚠️\x1b[0m\n" +
            "\x1b[33m🔍 All group messages will be exposed\x1b[0m\n" +
            "\x1b[33m⚠️  Use with extreme caution. ⚠️\n");
        console.info("Read the guidelines at https://messagekit.ephemerahq.com/guidelines\x1b[0m");
    }
